...
Canvas::Canvas(Target* t, color_t bg, BestCanvas* bc) : target(t), bgColor(bg),
  width(target->getWidth()), height(target->getHeight()), bestCanvas(bc)
{
...
Canvas::Canvas(Target* t,color_t bg_Red, color_t bg_Green, color_t bg_Blue,
    BestCanvas* bc) : target(t), bgColor_Red(bg_Red), bgColor_Green(bg_Green),
  bgColor_Blue(bg_Blue), width(target->getWidth()), height(target->getHeight()),
  bestCanvas(bc)
...
bool Canvas::pixelIsDirty(Coord c)
{
  ...
  if(reader->getColourMode())
  {
    ...
    if (pixel.r == bgColor_Red && pixel.g ==  bgColor_Green &&
        pixel.b == bgColor_Blue)
      return false;
  }
  else
  {
