...
RandFloat::RandFloat(GPConfig* conf) : Terminal(ReturnFloat::TYPENUM, "float",
    conf), value(config->randomNumGenerator->randReal() * 1.0)
{
}

RandFloat::RandFloat(float initValue, GPConfig* conf) :
  Terminal(ReturnFloat::TYPENUM, "float", conf), value(initValue)
{
}
