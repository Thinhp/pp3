...
private:
  ...
  int randomSeed;
  int canvasBG_R, canvasBG_G, canvasBG_B;
  float fitnessTarget;
...
public:
  ...
  const static int DRAW_MODE_PALETTE_SHROUD = 4;
  const static int PIXEL_STRATEGY_BLEND = 0;
  const static int PIXEL_STRATEGY_NO_PIXEL = 1;
  const static int PIXEL_STRATEGY_NO_STROKE = 2;
  const static int PIXEL_STRATEGY_STOP_STROKE = 3;
